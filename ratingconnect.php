...
class User {
	...
	private $dbPassword = "";
	private $dbName     = "movielens";
	private $userTbl    = 'users';

	
	function __construct(){
		$result1 = $_SESSION['result11'];
		echo $result1;
		echo "In COntrocutor UserConnect.php";
		if(!isset($this->db)){
	...
	function checkUser($userData = array()){
		if(!empty($userData)){
			...
			if($prevResult->num_rows > 0){
				...
				$query = "SELECT id FROM ".$this->userTbl." WHERE oauth_uid = '".$userData['oauth_uid']."'";
				//$update = $this->db->query($query);
				$result1 = $this->db->query($query);
				$ud = $result1->fetch_assoc();
				echo $ud['id'];
				echo "\nHello";
			}else{
				//Insert user data
